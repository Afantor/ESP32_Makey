...
// bird
const unsigned int BIRDCOL = Makey.Lcd.color565(255,254,174);
// pipe
const unsigned int PIPECOL  = Makey.Lcd.color565(99,255,78);
// pipe highlight
const unsigned int PIPEHIGHCOL  = Makey.Lcd.color565(250,255,250);
// pipe seam
const unsigned int PIPESEAMCOL  = Makey.Lcd.color565(0,0,0);
// floor
const unsigned int FLOORCOL = Makey.Lcd.color565(246,240,163);
// grass (col2 is the stripe color)
const unsigned int GRASSCOL  = Makey.Lcd.color565(141,225,87);
const unsigned int GRASSCOL2 = Makey.Lcd.color565(156,239,88);

// bird sprite
// bird sprite colors (Cx name for values to keep the array readable)
#define C0 BCKGRDCOL
#define C1 Makey.Lcd.color565(195,165,75)
#define C2 BIRDCOL
#define C3 TFT_WHITE
#define C4 TFT_RED
#define C5 Makey.Lcd.color565(251,216,114)
...
#define drawPixel(a, b, c) Makey.Lcd.setAddrWindow(a, b, a, b); Makey.Lcd.pushColor(c)
...
void game_loop() {
  ...
  // black line
  Makey.Lcd.drawFastHLine(0, GAMEH, TFTW, TFT_BLACK);
  // grass and stripe
  Makey.Lcd.fillRect(0, GAMEH+1, TFTW2, GRASSH, GRASSCOL);
  Makey.Lcd.fillRect(TFTW2, GAMEH+1, TFTW2, GRASSH, GRASSCOL2);
  // black line
  Makey.Lcd.drawFastHLine(0, GAMEH+GRASSH, TFTW, TFT_BLACK);
  // mud
  Makey.Lcd.fillRect(0, GAMEH+GRASSH+1, TFTW, FLOORH-GRASSH, FLOORCOL);
  ...
  while (1) {
    ...
    if (pipes.x >= 0 && pipes.x < TFTW) {
      // pipe color
      Makey.Lcd.drawFastVLine(pipes.x+3, 0, pipes.gap_y, PIPECOL);
      Makey.Lcd.drawFastVLine(pipes.x+3, pipes.gap_y+GAPHEIGHT+1, GAMEH-(pipes.gap_y+GAPHEIGHT+1), PIPECOL);
      // highlight
      Makey.Lcd.drawFastVLine(pipes.x, 0, pipes.gap_y, PIPEHIGHCOL);
      Makey.Lcd.drawFastVLine(pipes.x, pipes.gap_y+GAPHEIGHT+1, GAMEH-(pipes.gap_y+GAPHEIGHT+1), PIPEHIGHCOL);
    ...
    // erase behind pipe
    if (pipes.x <= TFTW) Makey.Lcd.drawFastVLine(pipes.x+PIPEW, 0, GAMEH, BCKGRDCOL);
    ...
    Makey.Lcd.drawFastVLine( grassx    %TFTW, GAMEH+1, GRASSH-1, GRASSCOL);
    Makey.Lcd.drawFastVLine((grassx+64)%TFTW, GAMEH+1, GRASSH-1, GRASSCOL2);
    ...
    // if bird has passed the pipe increase score
      ...
      Makey.Lcd.setTextColor(BCKGRDCOL);
      Makey.Lcd.setCursor( TFTW2, 4);
      Makey.Lcd.print(score);
      // set text color back to white for new score
      Makey.Lcd.setTextColor(TFT_WHITE);
...
// ---------------
void game_start() {
  Makey.Lcd.fillScreen(TFT_BLACK);
  Makey.Lcd.fillRect(10, TFTH2 - 20, TFTW-20, 1, TFT_WHITE);
  Makey.Lcd.fillRect(10, TFTH2 + 32, TFTW-20, 1, TFT_WHITE);
  Makey.Lcd.setTextColor(TFT_WHITE);
  Makey.Lcd.setTextSize(2);
  // half width - num char * char width in pixels
  Makey.Lcd.setCursor( TFTW2-(6*9), TFTH2 - 16);
  Makey.Lcd.println("FLAPPY");
  Makey.Lcd.setTextSize(2);
  Makey.Lcd.setCursor( TFTW2-(6*9), TFTH2 + 8);
  Makey.Lcd.println("-BIRD-");
  Makey.Lcd.setTextSize(2);
  Makey.Lcd.setCursor( 10, TFTH2 - 36);
  Makey.Lcd.println("Makey32");
  Makey.Lcd.setCursor( TFTW2 - (17*9), TFTH2 + 36);
...
void game_init() {
  // clear screen
  Makey.Lcd.fillScreen(BCKGRDCOL);
  // reset score
  score = 0;
  // init bird
  bird.x = 144;
...
void game_over() {
  ...
  if(score>maxScore)
  {
    ...
    maxScore = score;
    Makey.Lcd.setTextColor(TFT_RED);
    Makey.Lcd.setTextSize(2); 
    Makey.Lcd.setCursor( TFTW2 - (13*6), TFTH2 - 26);
    Makey.Lcd.println("NEW HIGHSCORE");
  }
  
  Makey.Lcd.setTextColor(TFT_WHITE);
  Makey.Lcd.setTextSize(3);
  // half width - num char * char width in pixels
  Makey.Lcd.setCursor( TFTW2 - (9*9), TFTH2 - 6);
  Makey.Lcd.println("GAME OVER");
  Makey.Lcd.setTextSize(2);
  Makey.Lcd.setCursor( 10, 10);
  Makey.Lcd.print("score: ");
  Makey.Lcd.print(score);
  Makey.Lcd.setCursor( TFTW2 - (12*6), TFTH2 + 18);
  Makey.Lcd.println("press button");
  Makey.Lcd.setCursor( 10, 28);
  Makey.Lcd.print("Max Score:");
