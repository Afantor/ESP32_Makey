...
void DisplayCh::fillScreen(uint32_t color) {
  ...
  Cubic.Lcd.fillRect(0, 0, _width, _height, color);
...
void DisplayCh::writeHzkAsc(const char c) {
  ...
  if (c == '\n') {
    ...
    if (highlighted) {
      ...
      Cubic.Lcd.fillRect(cursor_x, cursor_y, ascCharWidth, ascCharHeigth, highlightcolor);

    } else if (istransparent == false) {

      Cubic.Lcd.fillRect(cursor_x, cursor_y, ascCharWidth, ascCharHeigth, textbgcolor);
      ...
      for (uint8_t col = 0; col < 8; col++) {
        ...
        if ((*pCharMatrix & mask) != 0) {
          ...
          if (textsize == 1) {
            ...
            Cubic.Lcd.drawPixel(posX, posY, textcolor);

          } else {

            Cubic.Lcd.fillRect(posX, posY, textsize, textsize, textcolor);
...
void DisplayCh::writeHzkGbk(const char *c) {
  ...
  if (highlighted) {
    ...
    Cubic.Lcd.fillRect(cursor_x, cursor_y, gbkCharWidth, gbkCharHeight, highlightcolor);

  } else if (istransparent == false) {

    Cubic.Lcd.fillRect(cursor_x, cursor_y, gbkCharWidth, gbkCharHeight, textbgcolor);
    ...
    for (uint8_t col = 0; col < 8; col++) {
      ...
      if ((*(pCharMatrix + 1) & mask) != 0) {
        ...
        if (textsize == 1) {
          ...
          Cubic.Lcd.drawPixel(posX + ascCharWidth, posY, textcolor);

        } else {

          Cubic.Lcd.fillRect(posX + ascCharWidth, posY, textsize, textsize, textcolor);
