  ...
  class LCDDisplay : public TFT_eSPI {
    public:
      LCDDisplay();
      void begin();
      void sleep();
      ...
      void clearDisplay(uint32_t color=ST7789_BLACK) { fillScreen(color); }
      void clear(uint32_t color=ST7789_BLACK) { fillScreen(color); }
      void display() {}
      ...
      void qrcode(const char *string, uint16_t x = 10, uint16_t y = 10, uint8_t width = 220, uint8_t version = 6);
      void qrcode(const String &string, uint16_t x = 10, uint16_t y = 10, uint8_t width = 220, uint8_t version = 6);
