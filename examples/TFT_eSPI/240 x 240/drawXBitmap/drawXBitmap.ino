...
#include <Makey32.h>        // Hardware-specific library


void setup() {

  Makey.begin();               // Initialise the display

  Makey.Lcd.fillScreen(TFT_BLACK); // Black screen fill
}
...
void loop() {
  ...
  int x = random(Makey.Lcd.width()  - logoWidth);
  int y = random(Makey.Lcd.height() - logoHeight);
  ...
  Makey.Lcd.drawXBitmap(x, y, logo, logoWidth, logoHeight, TFT_WHITE);

  delay(500);

  // Erase old one by drawing over with background colour
  Makey.Lcd.drawXBitmap(x, y, logo, logoWidth, logoHeight, TFT_BLACK);


  // Example 2
  // =========
  // New random x and y coordinates
  x = random(Makey.Lcd.width()  - logoWidth);
  y = random(Makey.Lcd.height() - logoHeight);
  ...
  Makey.Lcd.drawXBitmap(x, y, logo, logoWidth, logoHeight, TFT_WHITE, TFT_RED);

  delay(500);

  // Erase old one by drawing over with background colour
  Makey.Lcd.drawXBitmap(x, y, logo, logoWidth, logoHeight, TFT_BLACK, TFT_BLACK);
