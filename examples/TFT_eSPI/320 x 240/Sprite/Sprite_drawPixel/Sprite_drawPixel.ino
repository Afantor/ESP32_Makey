...
TFT_eSprite spr = TFT_eSprite(&Cubic.Lcd);
...
void loop(void)
{
  ...
  spr.pushSprite(Cubic.Lcd.width() / 2 - WIDTH / 2, Cubic.Lcd.height() / 2 - HEIGHT / 2);
  spr.pushSprite(Cubic.Lcd.width() - WIDTH + 40, Cubic.Lcd.height() - HEIGHT + 40);
  ...
  int x = Cubic.Lcd.width() / 2  -  WIDTH / 2;
  int y = Cubic.Lcd.height() / 2 - HEIGHT / 2;
  ...
  while (true)
  {
    ...
    if (x < -WIDTH/2) dx = 1;
    if (x >= Cubic.Lcd.width()-WIDTH/2) dx = -1;
    if (y < -HEIGHT/2) dy = 1;
    if (y >= Cubic.Lcd.height()-HEIGHT/2) dy = -1;
