...
void loop() {
  ...
  for (int i = 199; i >= 0; i--) {
    yield(); tft.drawFloat(i/10.0, decimal_places, x, y + 70, font); // Use 1 decimal place
    delay(100); 
  }
  ...
  for (int i = 199; i >= 0; i--) {
    yield(); tft.drawNumber(i, x, y + 70, font); // Use 1 decimal place
    delay(100);
  }
...
void header(const char *string)
{
  ...
  tft.drawString(string, 120, 10, 4);
